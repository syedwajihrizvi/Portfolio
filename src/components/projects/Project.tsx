...
function Project({ information }: Props) {
  ...
  const handleInitialProjectInfoAnimationState = () => {
    if (infoButton.info) return { opacity: 0 };
    return { opacity: 1 };
  };

  const handleFinalProjectInfoAnimationState = () => {
    if (infoButton.info) return { opacity: 1 };
    return { opacity: 0 };
  };

  const handleInitialProjectUsageAnimationState = () => {
    if (infoButton.usage) return { opacity: 0 };
    return { opacity: 1 };
  };

  const handleFinalProjectUsageAnimationState = () => {
    if (infoButton.usage) return { opacity: 1 };
    return { opacity: 0 };
  };
  ...
  const renderVideos = () => {
    return (
      <Box className="project__videos">
        {information?.videos?.map((video) => (
          <video playsInline muted autoPlay loop src={video} style={{ width: "100%", height: "100%" }}></video>
        ))}
      </Box>
    );
  };
  ...
  const projectInfo = () => (
    <MotionBox
      initial={handleInitialProjectInfoAnimationState()}
    ...
    >
      <Typography className="project__info__text">{information.description}</Typography>
    </MotionBox>
  );

  const projectUsage = () => (
    <MotionBox
      initial={handleInitialProjectUsageAnimationState()}
        ...
        {information.usage.map((data) => (
          <>
            <ListItem className="list__item" alignItems="flex-start">
              <ListItemIcon>{renderIcon(data.icon)}</ListItemIcon>
              <ListItemText
                primary={<Typography fontWeight="900">{data.title}</Typography>}
                secondary={<Typography className="text">{data.description}</Typography>}
              />
            </ListItem>
        ...
        <Grid className="project__desc" item xs={12} md={6}>
          <Box className="project__desc__content">
            ...
            <Box className="project__buttons">
              ...
              {information.status && information.status == "coming__soon" && (
                <Button className="project__button button--primary" startIcon={<Timer />}>
                  Coming Soon
                </Button>
              )}
              ...
              <Button
                className="project__button button--info"
                startIcon={viewProjectInfo ? <SummarizeIcon /> : <InfoIcon />}
                onClick={handleInfoButtonSelect}
              >
